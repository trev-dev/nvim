...
(fn gitsigns-on-attach [bufnr]
  (let [gs package.loaded.gitsigns]
    ...
    (maplocal :n "[c" (λ [] (if vim.wo.diff "[c"
                                 (do
                                   (vim.schedule (λ [] (gs.prev_hunk)))
                                   "<Ignore>"))))
    (maplocal :n "]c" (λ [] (if vim.wo.diff "]c"
                                 (do
                                   (vim.schedule (λ [] (gs.next_hunk)))
                                   "<Ignore>"))))
    ...
    (maplocal :n "<leader>hb" (λ [] (gs.blame_line { :full true }))
                              { :desc "blame line" })
    (maplocal :n "<leader>tb" gs.toggle_current_line_blame 
